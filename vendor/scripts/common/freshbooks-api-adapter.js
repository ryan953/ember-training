...
DS.FreshBooksApiSerializer = DS.RESTSerializer.extend({

  extract: function(store, type, payload, x, findMethod) {
    var singleType = type.typeKey,
      pluralType = Ember.String.pluralize(singleType),
      fbkIdName = singleType + '_id',
      data = payload.response[pluralType][0][singleType];
    data.forEach(function(item) {
      if (item[fbkIdName]) {
        item.id = item[fbkIdName];
      }
    });
    return data;
  },
  ...
  normalize: function(type, hash, prop) {
    console.log('normalize', type, hash, prop);
    var json = {};
    for(var key in hash) {
      if(key !== 'links') {
        json[key] = hash[key];
...
DS.FreshBooksApiAdapter = DS.RESTAdapter.extend({

  defaultSerializer: 'DS/freshBooksApi',
  ...
  findAll: function(store, type, sinceToken) {
    var url = this.buildURL(type.typeKey) + '/list';
    return this.ajax(url, 'GET', {
      contentType: 'application/json',
      headers: {
        'Authorization': basicAuthHeader('ryan953', '5efe63f3fd641101ff9781620a2d7afc')
      }
    });
  },
  ...
  findMany: function(store, type, ids, owner) {
    console.log('hello world', FreshBooksAPI.Token);
